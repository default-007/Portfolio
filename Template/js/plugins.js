...
$(function () {
	...
	$(".aboutMe").on("scroll", function () {
		var offsetTop = getOffsetTop(
			$(this),
			$(".kayo-facts-about-me .kayo-info-list")
		);
	...
	$(document, window).on("scroll", function () {
		if (demo2Check || window.matchMedia("(max-width: 767px)").matches) {
			var bodyScrollTop = $(document, window).scrollTop(),
				listoffsetTop =
					$(".kayo-facts-about-me .kayo-info-list").offset().top -
					$(window).height() / 2;
	...
	testimonials_slider.on("init", function (slick, slider) {
		var totalSlides = slider.$slides.length,
			...
			prevSlideImg = $(slider.$slides[totalSlides - 1])
				.find("img")
				.attr("src");
		$("#testimonial-nextArrow .testimonials-client-page").css(
			"background-image",
			"url(" + nextSlideImg + ")"
		);
		$("#testimonial-prevArrow .testimonials-client-page").css(
			"background-image",
			"url(" + prevSlideImg + ")"
		);
	});
